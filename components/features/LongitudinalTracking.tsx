...
import { UploadCloud, Loader2, AlertCircle, Plus, Trash2, Calendar, Clock } from 'lucide-react';
import ReactMarkdown from 'react-markdown';
import { analyzeLongitudinal } from '@/lib/gemini';

export default function LongitudinalTracking() {
  const [images, setImages] = useState<{ id: string; base64: string; date: string }[]>([]);
  ...
  const handleDateChange = (id: string, newDate: string) => {
    setImages(images.map((img) => (img.id === id ? { ...img, date: newDate } : img)));
  };
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-900">Longitudinal Tracking</h1>
        <p className="text-slate-500 mt-1">Upload multiple photos of the same lesion over time to track evolution and flag concerning changes.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-slate-900">Lesion Timeline</h2>
            ...
            <div className="space-y-4">
              {images.map((img, index) => (
                <div key={img.id} className="flex gap-4 items-start p-4 border border-slate-100 rounded-xl bg-slate-50 relative group">
                  <div className="w-24 h-24 rounded-lg overflow-hidden border border-slate-200 shrink-0 relative">
                    <img src={img.base64} alt={`Lesion ${index + 1}`} className="w-full h-full object-cover" />
                  </div>
                  <div className="flex-1 space-y-2">
                    <label className="block text-xs font-medium text-slate-500">Date Taken</label>
              ...
              <div className="relative border-2 border-dashed border-slate-300 rounded-xl p-6 flex flex-col items-center justify-center bg-slate-50 hover:bg-slate-100 transition-colors cursor-pointer">
                ...
                <span className="text-sm font-medium text-slate-600">Add Timeline Image</span>
              </div>
            </div>
        ...
        <div className="lg:col-span-7">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm h-full min-h-[600px] flex flex-col">
            <h2 className="text-lg font-semibold text-slate-900 mb-4 border-b border-slate-100 pb-4">Evolution Report</h2>
            ...
            {result ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-slate-400">
                ...
                <p className="text-sm text-center max-w-xs">Upload at least two images over time and run the analysis to see the evolution report.</p>
              </div>
            )}
          </div>
        </div>
