...
export default function Dashboard() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-red-200">
      {/* Sidebar */}
      <aside className="w-64 bg-red-800 border-r border-slate-200 flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-1">
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`w-5 h-5 transition-colors ${isActive ? "text-black" : "text-white group-hover:text-black"}`}
                />
                {tab.label}
              </button>
            );
        ...
        <div className="p-4 border-t border-slate-100">
          <div className="flex items-center gap-3 px-3 py-2">
            <div className="w-8 h-8 rounded-full bg-black flex items-center justify-center text-xs font-medium text-slate-600">
              DR
            </div>
            <div className="flex flex-col text-left">
              <span className="text-sm font-medium text-white">Dr. Smith</span>
              <span className="text-xs text-white">Dermatologist</span>
            </div>
          </div>
        </div>
      </aside>
